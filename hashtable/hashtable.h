...
hashtable_t *ht_create(size_t tab_size,
    ...
    if((ht = (hashtable_t *) calloc(1, sizeof(*ht))) == NULL)
        {return NULL;}
...
ht_entry_t *ht_newpair(char *key, char *value) {
    ...
    if((newpair = (ht_entry_t *) calloc(1, sizeof(*newpair))) == NULL)
        {return NULL;}
    /* copy the key */
    if((newpair->key = strdup(key)) == NULL)
        {free(newpair); return NULL;}
    /* copy the value */
    if((newpair->value = strdup(value)) == NULL)
        {free(newpair->key); free(newpair); return NULL;}
 ...
 */
int ht_set(hashtable_t *ht, char *key, char *value) {
    int bin = 0;
    ht_entry_t *newpair = NULL;
